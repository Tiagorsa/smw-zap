...
const bot_version = 'v 0.230.2.18 (1317)'
...
client.on('message', async msg => {
  ...
  const opcoes_perfil = { 0: { 'menu': '0-Consulta Produto' } }
  ...
  if (allowed_users.includes(user)) {
    ...
    if (user in bot_memory == false) {
      ...
      stat = 0;
      local = 1;
      bot_memory[user] = { 'stat': stat, 'perfil': perfil, 'local': local, 'last_action': new Date().toString(), 'last_login': new Date().toString() };
    } else {
      stat = bot_memory[user]['stat'];
      perfil = bot_memory[user]['perfil'];
      local = bot_memory[user]['local'];
    }
    console.log('Usuário: ' + user + ' - ' + stat)
